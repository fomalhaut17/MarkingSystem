...
## Mock 서버만 실행하고 Visual Studio에서 앱 기동

Visual Studio에서 앱만 디버깅할 때는 Mock 서버를 별도로 먼저 실행합니다.

```
run-local-mock.bat
```

### Visual Studio — 여러 프로젝트 동시 시작 설정

Mock 서버와 앱을 Visual Studio에서 함께 F5로 실행하려면 아래와 같이 설정합니다.
...
| PLC 연결 실패 | Mock 서버 창이 닫혀 있음. bat 파일 재실행 |
